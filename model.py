...
def log2_graph(x):
    ...
    return tf.math.log(x) / tf.math.log(2.0) ##QS
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        keep = tf.sparse.to_dense(keep)[0]  #QS
    ...
    #keep = tf.sets.set_intersection(tf.expand_dims(keep, 0),
    keep = tf.sets.intersection(tf.expand_dims(keep, 0),               #QS
...
class MaskRCNN():
    ...
    def compile(self, learning_rate, momentum):
        ...
        def outputinloss(outp):
            if outp in tf.exp(self.keras_model.losses):
                return 1
            return 0
        added_loss_name = []
        for name in loss_names:
            ...
            #    continue
            #iscond = outputinloss(layer.output)
            #print("layer.output")
            #print(layer.output)
            #print("self.keras_model.losses")
            #print(tf.exp(self.keras_model.losses))
        ...
        for name in loss_names:
            ...
            loss = (
                tf.reduce_mean(layer.output, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.))
            self.keras_model.metrics.append(loss) ##self.keras_model.metrics_tensors.append(loss)
